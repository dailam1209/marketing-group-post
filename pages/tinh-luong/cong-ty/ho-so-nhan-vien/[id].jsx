...
const App = () => {
  ...
  const fetchApiData = () => {
    ...
    axios
        .post("http://210.245.108.202:3009/api/tinhluong/nhanvien/qly_ho_so_ca_nhan", {
          token: token,
          ep_id: ep_id,
          cp: cp,
  ...
  const handleDelete = async (sb_id) => {
    try {
      const response = await axios.post("http://210.245.108.202:3009/api/tinhluong/congty/delete_basic_salary", {
        token: token,
        sb_id: sb_id
      })
      if (response) {
  ...
  const handleOk = async () => {
    ...
    try {
      const response =
          await axios.post("http://210.245.108.202:3009/api/tinhluong/congty/insert_basic_salary", newTable)
  ...
  const handleEdit = async () => {
    ...
    try {
      const response =
          await axios.post("http://210.245.108.202:3009/api/tinhluong/congty/update_basic_salary", newTable)
  ...
  const columns = [
    {
      ...
      render: (record) => (
          <div className={styles.add_flex}>
            <p className={styles.p_style}>{record?.sb_salary_basic.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VNĐ</p>
          </div>
      ),
    },
    {
      ...
      render: (record) => (
          <div>
            <p className={styles.p_red}>{record?.sb_salary_bh.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VNĐ</p>
          </div>
      ),
    },
    {
      ...
      render: (record) => (
          <div>
            <p className={styles.p_red}>{record?.sb_pc_bh.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VNĐ</p>
          </div>
      ),
    },
    {
    ...
    },
    {
      title: "Sửa",
      render: (record) => (
          <button className={styles.button_style} onClick={() => {
  ...
  const handleEditUser = async () => {
    ...
    try {
      const response =
          await axios.post("http://210.245.108.202:3009/api/tinhluong/congty/edit_detail_inforuser", tableUser)
      console.log("Sửa thành công:", response);
      setIsClose(true);
      setIsEditOpen(false);
      setReload(!reload)
  ...
  const openEdit = () => {
    ...
    setGender((user?.info_dep_com?.user?.inForPerson?.account.gender === 1) ? "Nam" : "Nữ")
    setDate(dayjs.unix(user?.info_dep_com?.user?.inForPerson?.account?.birthday));
    setIdQlC(user?.info_dep_com?.user?.idQLC)
    setMarried((user.info_dep_com?.user?.inForPerson?.account.married === 1) ? "Độc thân" : "Đã kết hôn")
      ...
      <>
        <div className={styles.container_index}>
              ...
              <div>
                <div className={styles.name_content}>
                  <div>
                    <div className={styles.name}>
                      <div>
                        <Image alt="/" src={"/add.png"} width={110} height={110}
                               style={{borderRadius: "50%"}}/>
                        <Image alt="/" src={"/edit.png"} width={27} height={27}
                    ...
                    <button className={styles.name_button}>Thay đổi lịch làm việc</button>
                  </div>
                </div>
                ...
                <div className={styles.introduce_container}>
                  <div className={styles.introduce}>
                    <p>Giới thiệu</p>
                    <Image alt="/" src={"/edit.png"} width={27} height={27} onClick={openIntroduce}/>
                  </div>
                  <div>
                    <p className={styles.p_gt}>{dataUser?.info_dep_com?.user?.inForPerson?.employee?.ep_description}</p>
                  </div>
                  {isIntroduceOpen && (
                      <div>
                                        <textarea type="text" rows={8} className={styles.textarea}
                                                  placeholder="Thêm ghi chú">{dataUser?.info_dep_com?.user?.inForPerson?.employee?.ep_description}</textarea>
                        <div className={styles.btn_user}>
                          <button className={styles.input_cancer} onClick={closeIntroduce}>
                            Hủy
                          </button>
                          <button className={styles.input_submit}>Lưu thông tin</button>
                        </div>
                      </div>
                  )}
                </div>
                ...
                <div className={styles.information_user}>
                  ...
                  <Image alt="/" src={"/edit.png"} width={27} height={27} onClick={openEdit}/>
                </div>
                {isClose && (
                    <div className={styles.information}>
                      <div>
                        <p className={styles.information_p}>Họ và
                          tên: {dataUser?.info_dep_com?.user?.userName}</p>
                        <p className={styles.information_p}>Ngày
                          sinh: {new Date(dataUser?.info_dep_com?.user?.inForPerson?.account?.birthday * 1000).toLocaleDateString('en-GB')}</p>
                        <p className={styles.information_p}>Phòng
                          ban: {dataUser?.info_dep_com?.department?.dep_name}</p>
                        <p className={styles.information_p}>Địa
                          chỉ: {dataUser?.info_dep_com?.user?.address}</p>
                        <p className={styles.information_p}>Số điện
                          thoại: {dataUser?.info_dep_com?.user?.phone}</p>
                        <p className={styles.information_p}>Ngân hàng nhận
                          lương: {(dataUser?.info_emp_start?.st_bank) ? dataUser?.info_emp_start?.st_bank : "Chưa cập nhật"}</p>
                        <p className={styles.information_p}>Hồ sơ nhân viên: Xem chi tiết</p>
                      </div>
                      <div>
                        <p className={styles.information_p}>Giới
                          tính: {(dataUser?.info_dep_com?.user?.inForPerson?.account?.gender) === 1 ? "Nam" : (dataUser?.info_dep_com?.user?.inForPerson?.account?.gender) === 2 ? "Nữ" : "Khác"}</p>
                        <p className={styles.information_p}>Mã nhân
                          viên: {dataUser?.info_dep_com?.user?.idQLC}</p>
                        <p className={styles.information_p}>Chức
                          vụ: {(dataUser?.info_dep_com?.user?.postion?.description) ? dataUser?.info_dep_com?.user?.postion?.description : "Chưa cập nhật"}</p>
                        <p className={styles.information_p}>Ngày bắt đầu
                          làm: {new Date(dataUser?.info_dep_com?.user?.inForPerson?.employee?.start_working_time * 1000).toLocaleDateString('en-GB')}</p>
                        <p className={styles.information_p}>Email: {dataUser?.info_dep_com?.user?.email}</p>
                        <p className={styles.information_p}>Số tài khoản ngân
                          hàng: {(dataUser?.info_emp_start?.st_stk) ? dataUser?.info_emp_start?.st_stk : "Chưa cập nhật"}</p>
                        <p className={styles.information_p}>Bắt đầu tính lương: Từ ngày bắt đầu làm
                          việc</p>
                      </div>
                    </div>
                )}
                    ...
                    <div className={styles.information_flex}>
                      <div className={styles.render}>
                        <div className={styles.flex_input}>
                          ...
                          <input value={nameUser} type="text" placeholder="Họ và tên"
                      ...
                      <div className={styles.render}>
                        <div className={styles.flex_input}>
                          ...
                          <DatePicker value={date} format={dateFormat}
                      ...
                      <div className={styles.render}>
                        ...
                        <div className={styles.flex_input}>
                          <label className={styles.p}>Số tài khoản ngân hàng:</label>
                          <input value={bankAccount} type="number"
                      ...
                      <div className={styles.btn_top}>
                        ...
                        <button className={styles.input_submit} onClick={handleEditUser}>Lưu thông tin</button>
                      </div>
                    </div>
                )}
              </div>
          ...
          <div className="modal_times">
            <Modal className={styles.modal_times} title="Thêm lương mức cơ bản" visible={isModalOpen}
                   onCancel={handleCancel} footer={null}>
              <div className={styles.times_body}>
                <label className={styles.p}>Lương cơ bản *</label>
                <input type="number" placeholder="Nhập lương cơ bản" className={styles.input}
                ...
                <input type="number"
                       ...
                       className={styles.input} value={policyDescription}
                ...
                <DatePicker format={dateFormat} className={styles.times_month} value={policyTime}
          ...
          {/*sửa*/}
          <div>
            <Modal className={styles.modal_times} title="Sửa lương mức cơ bản" visible={modalOpen}
                   onCancel={cancelModalEdit} footer={null}>
              <div className={styles.times_body}>
                <label className={styles.p}>Lương cơ bản *</label>
                <input type="number" placeholder="Nhập lương cơ bản" className={styles.input}
                       value={policyName} onChange={(e) => setPolicyName(e.target.value)}/>
                ...
                <input
                    ...
                    onChange={(e) => setDecisionBase(e.target.value)}
                />
                <Button type="primary" className={styles.btn_saves} onClick={handleEdit}>
                  Lưu
                </Button>
              </div>
            </Modal>
