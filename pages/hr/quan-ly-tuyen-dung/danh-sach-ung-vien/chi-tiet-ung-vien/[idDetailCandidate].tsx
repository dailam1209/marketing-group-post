...
import { ContactJobDetails } from '@/pages/api/api-hr/quan-ly-tuyen-dung/candidateList'
import { GetJobDetails } from '@/pages/api/api-hr/quan-ly-tuyen-dung/candidateList'
import { FailJobDetails } from '@/pages/api/api-hr/quan-ly-tuyen-dung/candidateList'
import { CancelJobDetails } from '@/pages/api/api-hr/quan-ly-tuyen-dung/candidateList'
...
export default function DetailCandidate({ onCancel }: any) {
  ...
  const EmpMatchProcess = EmpData?.data?.find(
    (item: any) =>
      item.idQLC === isCandidateProcess?.detail_get_job?.empInterview ||
          ...
          if (data) {
            if (id?.includes('p')) {
              ...
              const item = data?.data?.find(
                (item: any) => (item.id = Number(matches[0]))
              )
              setCandidate(item)
            } else {
              const item = data?.data?.find(
                (item: any) => (item.id = Number(id?.slice(1, id?.length)))
              )
              setCandidate(item)
            }
          }
    ...
    const fetchData = async () => {
      try {
        ...
        if (response && id?.includes('p')) {
          ...
          const processFind = processData?.data?.find(
            (item: any) => item?.id === Number(numberAfterP)
          )
          if (processFind) {
  ...
  const selectedGender: any = options.chongioitinh.find(
    (item) => item.value === isCandidate?.gender
  )
  const selectedEducation: any = options.trinhdohocvan.find(
    (item) => item.value === isCandidate?.education
  )
  const selectedExp: any = options.kinhnghiemlamviec.find(
    (item) => item.value === isCandidate?.exp.toString()
  )
  const selectedMarried: any = options.tinhtranghonnhan.find(
    (item) => item.value === isCandidate?.isMarried
  )

  return (
    <>
      <Head>
        <title>Chi tiết hồ sơ ứng viên - Quản lý nhân sự - Timviec365.vn</title>
      </Head>
        ...
        <div className={`${styles.l_body_2}`}>
          <div className={`${styles.l_body_2_content} ${styles.l_body_2_left}`}>
            ...
            <div className={`${styles.l_body_2_left_body}`}>
              ...
              <p>
                Mã ứng viên: <span className={`${styles.txt_op}`}>UV{id}</span>
              </p>
              <p>
                Tên ứng viên:{' '}
                <span className={`${styles.txt_op}`}>{isCandidate?.name}</span>
              </p>
              <p>
                Email:{' '}
                <span className={`${styles.txt_op}`}>{isCandidate?.email}</span>
              </p>
              <p>
                Số điện thoại:{' '}
                <span className={`${styles.txt_op}`}>{isCandidate?.phone}</span>
              </p>
              <p>
                Giới tính:{' '}
              ...
              <p>
                ...
                <span className={`${styles.txt_op}`}>{selectedExp?.label}</span>
              </p>
              <p>
                Tình trạng hôn nhân:{' '}
                <span className={`${styles.txt_op}`}>
            ...
            <div className={`${styles.l_body_2_left_body}`}>
              ...
              <p>
                Vị trí ứng tuyển:{' '}
                <span className={`${styles.txt_op}`}>{isCandidate?.Title}</span>
              </p>
              <p>
                Nhân viên tuyển dụng:{' '}
                <span className={`${styles.txt_op}`}>
                  {isCandidate?.NvTuyenDung}
                </span>
              </p>
            </div>
            <div className={`${styles.l_body_2_left_body}`}>
                ...
                return (
                  <div
                    ...
                    <div
                      ...
                      style={{ marginTop: 10 }}>
                      <p style={{ display: 'inline-block', paddingRight: 20 }}>
                        {item?.skillName}:{' '}
                      </p>
                      <Rating
                        size={27}
                        disableFillHover
                        initialValue={item?.skillVote}
                        className={`${styles.star_rating}`}
                      />
                    </div>
                  </div>
            ...
            {id?.includes('p') && (
              <div className={`${styles.l_body_2_left_body}`}>
                ...
                <p>
                  ...
                  <span className={`${styles.txt_op}`}>{''}</span>
                </p>
                <p>
                  Mức lương mong muốn:{' '}
                  <span className={`${styles.txt_op}`}>{''}</span>
                ...
                <p>
                  Nhân viên tham gia:{' '}
                  <span className={`${styles.txt_op}`}>
                    {EmpMatchProcess?.userName}
                  </span>
                </p>
                <p>
                  Ghi chú: <span className={`${styles.txt_op}`}>{''}</span>
                </p>
              </div>
            )}
            {id?.charAt(0) === 'g' && (
          ...
          <div
            ...
            <p style={{ textAlign: 'center', color: '#4C5BD4', width: 'auto' }}>
