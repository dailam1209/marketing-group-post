...
import { LeaderBiographyDetail } from '@/pages/api/api-hr/co_cau_to_chuc'
import { LeaderBiograpphyUpdate } from '@/pages/api/api-hr/co_cau_to_chuc'
...
function Input_textarea({
  ...
  useEffect(() => {
    ...
    const initialValueElements = initialValueWithoutSecondAndThird?.map(
      (item: any) => {
...
export default function DetailLeaderBiography({ onCancel }: any) {
  ...
  useEffect(() => {
    if (idDetailLeaderBiography) {
      // Chỉ gọi API nếu idDetailLeaderBiography đã có giá trị
      const fetchData = async () => {
        try {
  ...
  const initialValue = [
    ...
    {
      key: 'Chuyên môn nghiệp vụ:',
      value: valuesArray[5] ? valuesArray[5] : '',
    },
      ...
      <div className={`${styles.l_body}`}>
        ...
        <div className={`${styles.l_body_2}`}>
          <div
            ...
            style={{ width: addField ? '41.677%' : '100%' }}>
            ...
            <div className={`${styles.l_body_2_left_body}`}>
              <div
                className={`${styles.d_info_detail}`}
                style={{ padding: 20 }}>
                <p className={`${styles.d_info_name}`}>
                  <span style={{ textTransform: 'uppercase', fontWeight: 600 }}>
                    {isLeaderDetail?.result?.namePosition}
                  </span>
                </p>
                <div
                  className={`${styles.d_images_info}`}
                  onClick={handleUploadClick}>
                  <img
                    src='/logo_com.png'
                    alt=''
                    className={`${styles.d_img_info}`}
                    id='preview_image'
                  />
                  <img
                    src='/camera.svg'
                    alt=''
                    className={`${styles.d_img_info1}`}
                  />
                  <input
                    type='file'
                    id='upload_img'
              ...
              <div className={`${styles.click_update}`} style={{ padding: 20 }}>
                <div
                  className={`${styles.d_info_detail2}`}
                  style={{ display: isOpenUpdate ? 'none' : 'block' }}>
                  {initialValue.map((item, index) => (
                    <p
                      key={index}
                      className={`${
                        item?.key === 'Thông tin cá nhân' ||
                        item?.key === 'Trình độ đào tạo'
                ...
                <div
                  className={`${styles.d_info_textarea1}`}
                  style={{ display: isOpenUpdate ? 'block' : 'none' }}>
                  <div
                    className={`${styles.d_info_detail3}`}
                    style={{ marginTop: 22 }}>
                    {isOpenUpdate && (
                      <Input_textarea
          ...
          <div
            ...
            style={{
              paddingTop: 0,
              paddingLeft: 15,
              display: addField ? 'inline' : 'none',
              width: addField ? '53%' : '0',
            }}>
            <div className={`${styles.d_info_header}`}>
              <input
                type='text'
                className={`${styles.d_input_info}`}
                id='d_input_name'
