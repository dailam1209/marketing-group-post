...
export default function HomeCompany() {
      ...
      try {
        ...
        if (parseInt(seconds) > parseInt(response.data.createdAt)) {
          ...
          if (response.data.com_vip === 1) {
            ...
            setVip(
              "Tài khoản công ty bạn chưa phải là tài khoản VIP! Tài khoản của bạn chỉ đăng ký tối đa 5 nhân viên"
            );
          }
        }
      } catch (error) {
  ...
  const [isRoleExists, setRoleExists] = useState(false);

  useEffect(() => {
    const tokenExists = !!Cookies.get("token_base365");
    const rfTokenExists = !!Cookies.get("rf_token");
    const roleExists = !!Cookies.get("role");
    const phoneExists = !!Cookies.get("phone");
    console.log(tokenExists, rfTokenExists, roleExists, phoneExists);
    setRoleExists(roleExists);
    if (roleExists || rfTokenExists) {
      console.log("Cookies still exist");
    } else {
              ...
              <div className="ctn_res_qly">
                <div className="left_header_qly">
                  <p className="share_clr_one share_fsize_one">
                    Ứng dụng / <span className="thay_doi">{nameList}</span>
                  </p>
                </div>
                  ...
                  <div className="tbao_ncap">
                    ...
                    <div className="nang_capv">
                      <p className="share_fsize_one ncap_tkhoan">
                        <a href="/thong-bao-tai-khoan-vip.html">
                          <img src="/img/napcap_vip.png" className="ncap_vip" />{" "}
                          Nâng cấp tài khoản
                        </a>
                      </p>
                    </div>
                ...
                <div className="list_all_qly">
                  ...
                  <div className="ctn_chile_tow">
                    {show == "all" ? (
                      <>
                        <QlNhanluc checkCookie={isRoleExists} />
                        <QlCongviec />
                        <QlNoibo />
                        <QlBanhang />
                      </>
