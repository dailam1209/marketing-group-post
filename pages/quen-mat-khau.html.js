...
export default function forgetPassword() {
  ...
  useEffect(() => {
    ...
    if (type == 1 || type == 2) {
      getTitle(
        'Khôi phục ngay tài khoản với tính năng lấy lại mật khẩu dễ dàng'
      )
      getDes(
        'Dễ dàng để Lấy lại mật khẩu cho tài khoản chuyển đổi số nhân viên khi bạn quên. Mật khẩu được khôi phục rất nhanh, đảm bảo quá trình sử dụng hiệu quả.'
      )
    } else {
      getTitle('Chuyển đổi số nhanh, khôi phục mật khẩu không lo chậm')
      getDes(
        'Trong 1 nốt nhạc, doanh nghiệp bạn có thể lấy lại mật khẩu tài khoản chuyển đổi số lỡ quên. Click tại Quên mật khẩu ngay và làm theo hướng dẫn vô cùng đơn giản.'
      )
    }
  }, [])
  // handle validate
  ...
  const onSubmit = async (data) => {
    ...
    if (response.result === true) {
      window.location.href =
        '/xac-thuc-ma-otp-mk.html?account=' + data.account + '&type=' + type
    } else {
                    ...
                    <div className='header_qmk'>
                      {type == 1 ? (
                        <h1 className='share_clr_four cr_weight_bold tex_center qlc_tieude_moi'>
                          Nhanh chóng, dễ dàng khôi phục tài khoản chuyển đổi số
                          cho doanh nghiệp
                        </h1>
                      ) : (
                        <h1 className='share_clr_four cr_weight_bold tex_center qlc_tieude_moi'>
                          Lấy lại mật khẩu tài khoản chuyển đổi số nhân viên cực
                          dễ, cực an toàn
                        </h1>
                      )}
