import Contract_go from "@/components/van-thu-luu-tru/management_dispatch/contract_go/Contract_go";
...
const Index = ({ data }: any) => {
  ...
  return <Contract_go data={listItemsNew} total_vb={data?.data?.count} />;
};
...
export async function getServerSideProps(context: {
  ...
  try {
    ...
    const currentPage = `${cookies.pageHDdi}`;
    const ItemsPerPage = `${cookies.pageSizeHDdi}`;
    if (!token) {
      return {
        redirect: {
          destination: "/404",
    ...
    const data = await fetchData(
      token,
      "/api/vanthu/contract/getListContract",
