import Contract_to from "@/components/van-thu-luu-tru/management_dispatch/contract_to/Contract_to";
...
const Index = ({ data }: any) => {
  ...
  return <Contract_to data={listItemsNew} total_vb={data?.data?.count} />;
};
...
export async function getServerSideProps(context: {
  ...
  try {
    ...
    const currentPage = `${cookies.pageHD}`;
    const ItemsPerPage = `${cookies.pageSizeHD}`;
    if (!token) {
      return {
        redirect: {
          destination: "/404",
