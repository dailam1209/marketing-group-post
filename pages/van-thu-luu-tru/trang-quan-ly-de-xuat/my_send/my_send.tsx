import Day_top_dexuat from "@/components/van-thu-luu-tru/components/day_top/Day_top_dexuat";
import Table_dexuat from "@/components/van-thu-luu-tru/components/incoming_doc_table/Table_dexuat";
...
import { fetch_employee, fetch_toiguidi } from "@/utils/api/dexuat/api_fetch";
...
const My_send = () => {
  ...
  const handleFilter = async (id: number) => {
    ...
    const response = await fetch_toiguidi({type : id, page: 1})
  ...
  const handlePageChange = async (e:any) => {
    const response = await fetch_toiguidi(
      {
        page : e.selected + 1,
  ...
  const handleSubmitFilter = async (e:any) => {
    ...
    const response = await fetch_toiguidi(form_data)
    setTableRecords(response?.data.data)
    setTotalpage(response?.data.totalPages)
  }
