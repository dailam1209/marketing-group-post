...
const ItemDropdown = (
  ...
  const [color,setColor] = useState('#474747')
    ...
    return (
      <a onClick={onClick} onMouseOver={()=>setColor(key==='5'?'#FF5B4D':'#4C5BD4')} onMouseOut={()=>setColor('#474747')}>
      <div
        key={key}
        style={{ display: "flex", alignItems: "top", margin: "5px 0 5px 0",color:color}}
      >
  ...
  return (
    <Col sm={12} xs={24}>
      <Card
        ...
        bordered={false}
        headStyle={{ backgroundImage: "url(/licklamviec.png)",backgroundSize: 'cover' }}
        key={key}
      >
...
export default function CaiDatLichLamViec(data: any, listCalendar: any) {
  ...
  return (
    <Row gutter={[29, 20]} className={styles.bodyCell}>
      ...
      {ThemNhanVien(modalThemNV, setModalThemNV, setModalUpload, cySelected, listEmp, listEmpInCy)}
      ...
      {XoaLich(modalXoaLich, setModalXoaLich, "Bạn chắc chắn muốn xóa lịch làm việc này?", cySelected)}
      {ModalChinhSua(modelChinhSua, setModalChinhSua, cySelected, setCySelected)}
    </Row>
  )
}
