...
export default function EmployeePolicy({ iconAdd, iconEdit, iconDelete }) {
  ...
  const setIdParent = async (itemId: number) => {
    const dataChildIndex = dataChildList.findIndex(
    ...
    if (dataChildIndex !== -1) {
      ...
      try {
        ...
        const updatedDataChildList = [
          ...dataChildList,
          { id: itemId, data: dataResponse },
        ]
          ...
          <div className={`${styles.tab_pane} ${styles.fade} `}>
            <div className={`${styles.recruitment2}`}>
              <div className={`${styles.recruitment2_3}`}>
                ...
                {openModal === 1 && (
                  <AddEmployeePolicyModal
                    onCancel={handleCloseModal}></AddEmployeePolicyModal>
                )}
                {openModal === 2 && (
                  <AddEmployeePolicyModal2
                    onCancel={handleCloseModal}></AddEmployeePolicyModal2>
                )}
                {isOpenDetailGroup !== 0 && (
                  <PolicyGroupDetailModal
                    idGroup={isOpenDetailGroup}
                    onCancel={handleCloseModal}></PolicyGroupDetailModal>
                )}
                {isOpenUpdateGroup !== 0 && (
                  <UpdatePolicyGroupsModal
                    idGroup={isOpenUpdateGroup}
                    onCancel={handleCloseModal}></UpdatePolicyGroupsModal>
                )}
                {isOpenDeleteGroup !== 0 && (
                  <DeletePolicyGroup
                    idGroup={isOpenDeleteGroup}
                    onCancel={handleCloseModal}></DeletePolicyGroup>
                )}
                {isOpenDetail !== 0 && (
                  <PolicyDetailModal
                    idGroup={isOpenDetail}
                    onCancel={handleCloseModal}></PolicyDetailModal>
                )}
                {isOpenDeletePolicy !== 0 && (
                  <DeletePolicys
                    idGroup={isOpenDeletePolicy}
                    onCancel={handleCloseModal}></DeletePolicys>
                )}
              </div>
                ...
                data?.data?.map((item: any, index: any) => (
                  <div className={`${styles.quydinh_item} `} key={index}>
                    <div className={`${styles.quydinh_item1}`}>
                      <div
                        className={`${styles.quydinh_item2}`}
                      ...
                      <a
                        ...
                        onClick={() => handleOpenPolicyGroupDetail(item?.id)}>
                        Chi tiết/
                      </a>
                      {iconEdit && (
                        <a
                          ...
                          onClick={() =>
                            handleOpentPolicyGroupUpdate(item?.id)
                          }>
                          Sửa/
                        </a>
                      )}
                      {iconDelete && (
                        <a
                          ...
                          onClick={() =>
                            handleOpentPolicyGroupDelete(item?.id)
                          }>
                          Xóa
                        </a>
                      )}
                    ...
                    <div
                      className={`${styles.table_none}`}
                      style={{
                        display: selectedItems.includes(item?.id)
                          ? 'block'
                          : 'none',
                      }}>
                      <table
                        className={`${styles.tablelist}  ${styles.tablelist1}`}>
                        <thead>
                          <tr>
                            <th scope='col'>STT</th>
            ...
            <div
              ...
              style={{ display: data?.tongSoBanGhi != 0 ? 'block' : 'none' }}>
              <MyPagination
                current={currentPage}
