...
export default function UpdateRegulationsModal({
  onCancel,
  idGroup,
}: UpdateRegulationsModal2Props) {
  const [provisionFile, setProvisionFile] = useState<File | null>(null)
  const [descriptions, setDescription] = useState('')
  const [ListRegulationsGroup, setListRegulationsGroup] = useState<any | null>(
    null
  )
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const provision_id = (
        document.getElementById('provision_id') as HTMLInputElement
      )?.value
      const time_start = (
        document.getElementById('time_start') as HTMLInputElement
      )?.value
      const supervisor_name = (
        document.getElementById('supervisor_name') as HTMLInputElement
      )?.value
      const apply_for = (
        document.getElementById('apply_for') as HTMLInputElement
      )?.value
          ...
          <div className={` ${styles.modal_dialog} ${styles.content_process}`}>
            <div className={`${styles.modal_content}`} ref={modalRef}>
              ...
              <form action=''>
                {DetailData?.data[0] && (
                  <div
                    className={`${styles.modal_body} ${styles.body_process}`}>
                    <div className={`${styles.form_groups}`}>
                      <label htmlFor=''>
                        Tên quy định <span style={{ color: 'red' }}> * </span>
                      </label>
                      <div className={`${styles.input_right}`}>
                        <input
                          type='text'
                          defaultValue={DetailData?.data[0]?.name}
                          id='names'
                    ...
                    <div className={`${styles.form_groups}`}>
                      ...
                      <div className={`${styles.input_right}`}>
                        <select
                          ...
                          id='provision_id'
                          className={`${styles.input_process}`}>
                          {ListRegulationsGroup?.data?.map(
                            (item: any, index: any) => (
                              <option
                                selected={
                                  item.id === DetailData?.data[0]?.provisionId
                                }
                                value={item.id}
                                key={index}>
                                -- {item.name} --
                    ...
                    <div className={`${styles.form_groups}`}>
                      <label htmlFor=''>
                        Người giám sát <span style={{ color: 'red' }}> * </span>
                      </label>
                      <div className={`${styles.input_right}`}>
                        <input
                          type='text'
                          value={DetailData?.data[0]?.supervisorName}
                          id='supervisor_name'
                    ...
                    <div className={`${styles.form_groups}`}>
                      ...
                      <div className={`${styles.input_right}`}>
                        <input
                          type='text'
                          value={DetailData?.data[0]?.applyFor}
                          id='apply_for'
                          placeholder='Đối tượng thi hành'
                          className={`${styles.input_process}`}
                        />
                      </div>
                    </div>
                    <div className={`${styles.form_groups} ${styles.cke}`}>
                      ...
                      <div className={`${styles.ckeditor}`}>
                        <Input_textarea
                          ...
                          content={DetailData?.data[0]?.content}
                        />
                      </div>
                    </div>
                    <div className={`${styles.form_groups}`}>
