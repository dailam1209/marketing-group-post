...
export default function EditPlanningModal({ onCancel, infoList }: any) {
  ...
  const [isPosition_idnew, setPosition_idnew] = useState<any>(
    infoList?.new_position_id
  )
  const [isDep_idNew, setDep_idNew] = useState<any>(infoList?.new_dep_id)
  const [isSpecified_idnew, setSpecified_idnew] = useState<any>(
    infoList?.decision_id
  )
  ...
  useEffect(() => {
    if (isSpecifiedList && infoList) {
      const foundSpecified = isSpecifiedList?.data?.find(
        (item: any) => item.id === Number(infoList?.decision_id)
      )
      if (foundSpecified) {
  ...
  const validationSchema = Yup.object().shape({
    ...
    created_at: Yup.string().required(
      'Vui lòng chọn thời gian quy hoạch bổ nhiệm'
    ),
  ...
  const options = {
    ...
    chonquydinh: [
      {
        value: infoList?.decision_id,
        label: isSpecified_name && isSpecified_name,
      },
    ],
                    ...
                    <div className={`${styles.input_right}`}>
                      {isSpecified_name && (
                        <Select
                          ...
                          onChange={(option) =>
                            handleSelectChange(option, setSpecified_idnew)
                          }
