...
import {
  FetchDataDep,
  FetchDataOrganizationalStructure,
  FetchDataEmployee,
  FetchDataPosition,
  FetchDataSpecifiedGroup,
} from '@/components/hr/util/listAll'
...
export default function AddWorkingModal({ onCancel }: any) {
  ...
  const [isOrganizationalStructureList, setOrganizationalStructureList] =
    useState<any>(null)
  ...
  useEffect(() => {
    const foundItem = isDepList?.data?.find(
      (item: any) => item.dep_id === isDep_id
    )
    if (foundItem) {
  ...
  const validationSchema = Yup.object().shape({
    ...
    created_at: Yup.string().required(
      'Vui lòng chọn thời gian luân chuyển công tác'
    ),
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const created_at = (
        document.getElementById('created_at') as HTMLInputElement
      )?.value
  ...
  function isValidComId(com_id: any) {
    return (
      isOrganizationalStructureList?.infoCompany?.parent_com_id === com_id ||
      isOrganizationalStructureList?.infoChildCompany?.some(
        (company) => company.com_id === com_id
      )
    )
  }
  ...
  const chonchinhanhOptions = useMemo(
    () =>
      companyNames &&
      companyNames?.map((organizational: any) => ({
  ...
  if (PositionList) {
    ...
    if (employee && Array.isArray(PositionList)) {
      ...
      const position = PositionList.find(
        (pos: any) => pos.positionId === position_id
      )
      if (position) {
  ...
  const options = {
    ...
    chonnhanvien:
      chonnhanvientheophongOptions?.length === 0
        ? chonnhanvienOptions
        : chonnhanvientheophongOptions,
                ...
                <form action=''>
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        ...
                        onChange={(option) =>
                          handleSelectChange(option, setCom_id)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setDep_id)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setInfoList)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setPosition_id)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setCom_idNew)
                        }
                        options={options.chonchinhanh}
                        placeholder='Chọn chi nhánh'
                        styles={{
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setDep_idNew)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectionChange(option, options.to)
                        }
                        options={options.to}
                        placeholder='Chọn tổ'
                        styles={{
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectionChange(option, options.nhom)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setPosition_idNew)
                        }
                  ...
                  <div className={`${styles.form_groups}`}>
                    ...
                    <div className={`${styles.input_right}`}>
                      <Select
                        defaultValue={selectedOption}
                        onChange={(option) =>
                          handleSelectChange(option, setSpecified_id)
                        }
                    ...
                    <div className={`${styles.ckeditor}`}>
                      <Input_textarea
                        onDescriptionChange={(data) =>
                          handleInputAreaChange(data, setMission)
                        }
                      />
                    </div>
                  </div>
                  <div className={`${styles.form_groups} ${styles.cke}`}>
                    ...
                    <div className={`${styles.ckeditor}`}>
                      <Input_textarea
                        onDescriptionChange={(data) =>
                          handleInputAreaChange(data, setNote)
                        }
                      />
                    </div>
                  </div>
