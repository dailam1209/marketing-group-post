...
export default function AddPayrollModal({ onCancel }: any) {
    ...
    if (isPositionList && Array.isArray(isPositionList.data)) {
      ...
      if (PositionList && isPosition_id) {
        const position = PositionList.find(
          (pos: any) => pos?.positionId === isPosition_id
        )
        if (position) {
  ...
  const options = {
    ...
    chonphongban: [
      {
        value: isDep_id && isDep_id,
        label: isDep_name ? isDep_name : 'Chọn phòng ban',
      },
    ],
    chonnhanvien: chonnhanvienOptions && chonnhanvienOptions,
    chucvuhientai: [
      {
        value: isPosition_id && isPosition_id,
        label: isPosition_name ? isPosition_name : 'Chọn chức vụ ',
      },
    ],
