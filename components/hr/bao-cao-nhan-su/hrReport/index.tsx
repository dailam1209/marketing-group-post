...
export default function TabHRReport({ dateRangeDatas }: any) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const formData = new FormData()
        const start_month = (
          document.getElementById('start_month') as HTMLInputElement
        )?.value
        const end_month = (
          document.getElementById('end_month') as HTMLInputElement
        )?.value
        formData.append('depId', isDep_id)
  ...
  const handleSearch: MouseEventHandler<HTMLAnchorElement> = (event) => {
    ...
    const endMonth = (document.getElementById('end_month') as HTMLInputElement)
      ?.value
        ...
        <div className={`${styles.content}`}>
              ...
              <div className={`${styles.search_new_t}`}>
                <div className={`${styles.div_no_pad}   `}>
                  <Select
                    ...
                    onChange={(option) =>
                      handleSelectChange(option, setIsDep_id)
                    }
                ...
                <div className={`${styles.div_no_pad}  `}>
                  <select
                    name=''
                    id=''
                    className={`${styles.form_control} ${styles.chart_type}`}
                    onChange={(event) => {
                ...
                <div
                  className={`${styles.div_no_pad} ${styles.div_no_pad_input}  `}
                  style={{ display: isSelect === 'date' ? 'block' : ' none' }}>
                  <input
                    type='date'
                ...
                <div
                  className={`${styles.div_no_pad} ${styles.div_no_pad_input}   `}
                  style={{ display: isSelect === 'month' ? 'block' : ' none' }}>
                  <input
                    type='month'
                    id='start_month'
                    className={`${styles.search_date} ${styles.form_control}`}
                ...
                <div
                  className={`${styles.div_no_pad} ${styles.div_no_pad_input}   `}
                  style={{ display: isSelect === 'year' ? 'block' : ' none' }}>
                  <Select
                    defaultValue={selectedOption}
                    onChange={(option) =>
                      handleSelectChange(option, setStartYear)
                    }
                ...
                <div
                  ...
                  <Select
                    defaultValue={selectedOption}
                    onChange={(option) =>
                      handleSelectChange(option, setEndYear)
                    }
                    options={options.chonnam}
                    placeholder='Chọn năm'
                    styles={{
                ...
                <div className={`${styles.div_no_pad_search}   `}>
                  <a
                    href=''
                    className={`${styles.icon_search_top} ${styles.div_search_hrreport} `}
                    onClick={handleSearch}>
                    <img
                      style={{ verticalAlign: 'middle', height: 30 }}
          ...
          <div className={`${styles.content_body}`}>
            ...
            <div className={`${styles.body_section3}`}>
              <InfomationSection3
                hrReportList={ReportChart}
                dateInRange={
                  dateRangeData.length === 0 ? dateRangeDatas : dateRangeData
                }
              />
            </div>
          </div>
