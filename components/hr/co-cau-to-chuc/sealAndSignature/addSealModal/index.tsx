...
export default function AddSealModal({ onCancel }: any) {
  ...
  const handleAddUserSignature = async (
  ...
  const handleSelectChangeDepartment = (
  ...
  const handleSelectChangeEmployee = (
    selectedOption: SelectOptionType | null
  ) => {
    setSelectedOption(selectedOption)
    if (selectedOption) {
  ...
  const filteredEmployees = useMemo(
    () =>
      EmployeeLists?.data?.filter(
        (emp: any) =>
          emp.dep_id[0] === isDep_id && emp.position_id === isPosition_id
      ),
            ...
            <div className={`${styles.modal_content}`} ref={modalRef}>
              <div
                ...
                <h5 className={`${styles.modal_tittle}`}>
                  THÊM MỚI NHÂN VIÊN ĐƯỢC SỬ DỤNG DẤU CÔNG TY
                </h5>
              </div>
              <form action=''>
                ...
                <div
                  ...
                  <button
                    ...
                    onClick={handleAddUserSignature}>
                    Thêm
                  </button>
                </div>
              </form>
