...
export default function SealAndSignature({
  iconAdd,
  iconEdit,
  iconDelete,
}: any) {
  ...
  const handleSelectChangeSignature = (
  ...
  return (
    <>
      <Head>
        <title>
          Quyền sử dụng con dấu và mẫu chữ ký - Quản lý nhân sự - Timviec365.vn
        </title>
      </Head>
        ...
        <div className={`${styles.tab_pane}`}>
          <div className={`${styles.body} ${styles.body_planning}`}>
            <div className={`${styles.recruitment}`}>
              <div className={`${styles.body_header_left}`}>
                <p style={{ color: '#4C5BD4', fontWeight: 600, fontSize: 14 }}>
                  Danh sách thành viên được sử dụng con dấu công ty
                </p>
              </div>
              <div className={`${styles.body_header_left}`}>
                {iconAdd && (
                  <button
                    className={`${styles.add} ${styles.add_planning}`}
            ...
            {openModal === 1 && (
              <AddSealModal onCancel={handleCloseModal}></AddSealModal>
            )}
            {openDeleteSealList !== 0 && (
              <DeleteSealUseList
                empId={openDeleteSealList}
                onCancel={handleCloseModal}
              />
            )}
            {openDeleteSignature !== 0 && (
              <DeleteSignatures
                empId={openDeleteSignature}
                onCancel={handleCloseModal}
              />
            )}
            <div className={`${styles.bg_search}`}>
              <div className={`${styles.search_new_t}`}>
                <div
                  className={`${styles.div_no_pad} ${styles.div_no_pad_planning} `}>
                  <input
                    id='emp_id'
                    type='text'
                    onChange={(event) => setKeySeal(event.target.value)}
                    className={`${styles.search_date} ${styles.form_control}`}
                ...
                <div className={`${styles.div_no_pad_search}   `}>
                  <a
                    ...
                    onClick={handleSearchSeal}>
                    <img
                      style={{ verticalAlign: '-webkit-baseline-middle' }}
                      src={`/t-icon-search-n.svg`}
                      alt=''
                    />
                  </a>
                </div>
                ...
                <table className={`${styles.table} ${styles.table_list}`}>
                  ...
                  <tbody className={`${styles.filter_body}`}>
                    {useSealList?.listEmpUseSignature?.length ? (
                      useSealList?.listEmpUseSignature.map(
                        (item: any, index: any) => (
                          <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{item?._id}</td>
                            ...
                            {iconDelete && (
                              <td>
                                <a
                                  ...
                                  onClick={() =>
                                    setOpenDeleteSealList(item?._id)
                                  }
          ...
          <div className={`${styles.body} ${styles.body_planning}`}>
            ...
            <div
              ...
              <div className={`${styles.table_content}`} ref={tableContentRef}>
                <table
                  className={`${styles.table} ${styles.table_list} ${styles.table_list2} `}>
                  <thead>
                    <tr>
                      <th>STT</th>
                      <th>ID</th>
                      ...
                      <th>Phòng ban</th>
                      <th>Mẫu chữ kí</th>
                      {iconEdit || iconDelete ? <th></th> : null}
                    </tr>
                  </thead>
                  <tbody className={`${styles.filter_body}`}>
                            ...
                            <td>
                              <label className={`${styles.custom_file_upload}`}>
                                <a
                                  href=''
                                  onClick={(event) =>
                                    handleUploadClick(event, item?._id)
                                  }>
                              ...
                              <input
                                ...
                                type='file'
                                onChange={(event) =>
                                  handleProvisionFileChange(event)
                                }></input>
                            </td>
                            {iconDelete || iconEdit ? (
                              <td>
                                {iconEdit && (
                                  <label
                                    htmlFor=''
                                    className={`${styles.edit_stamp}`}>
                                    <a
                                      href=''
                                      onClick={(event) =>
                                        handleEditClick(event, item?._id)
                                      }
                                ...
                                {iconDelete && (
                                  <a
                                    ...
                                    className={`${styles.btn_delete}`}
                                    onClick={() =>
                                      setOpenDeleteSignature(item?._id)
                                    }>
                                    <img src={`/icon_delete.svg`} alt='' />
                                  </a>
                                )}
