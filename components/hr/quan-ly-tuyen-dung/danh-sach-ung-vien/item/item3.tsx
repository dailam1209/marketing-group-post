...
export default function ItemCandidate3({
  process_id,
  setProcess_id,
  type,
  data,
  ...
  const handleClickDetail = (item: number) => {
    if (typeof item === 'number' && !isNaN(item)) {
      router.push(
        `/hr/quan-ly-tuyen-dung/danh-sach-ung-vien/chi-tiet-ung-vien/${type}${item}`
      )
    }
  }
  ...
  return (
    <>
      <div
        ...
        <div
          className={`${styles.hs_body_card}`}
          style={{ position: 'relative', left: 0, top: 0 }}>
          <div className={`${styles.row}`}>
            <div className={`${styles.hs_body_card_no1}`}>
              <img
                className={`${styles.hs_avt}`}
                src='https://phanmemnhansu.timviec365.vn/assets/images/t_images/logo_com.png'
                alt=''
              />
            </div>
            <div className={`${styles.hs_body_card_no2}`}>
              <p className={`${styles.word_wrap}`}>{data?.canName}</p>
              <p className={`${styles.word_wrap}`}>
                SĐT: <span>{data?.phone}</span>
              </p>
              <p className={`${styles.word_wrap}`}>{data?.title}</p>
              <Rating
                size={27}
                initialValue={data?.starVote}
                disableFillHover
            ...
            <div className={`${styles.hs_body_card_no3}`}>
              <a
                className={`${styles.hs_dot} ${styles.hs_dot1}`}
                style={{ cursor: 'pointer' }}>
                <img
                  onClick={() => handleToggleOption(data?.canId)}
                  src='https://phanmemnhansu.timviec365.vn/assets/images/t_images/hs-t-dot.svg'
                />
                <div
                  ref={modalRef}
                  className={`${styles.choose_option} ${styles.choose_option1}`}
                  style={{
                    display: isOpenOption || isDelete ? 'block' : 'none',
                  }}>
                  <ul style={{ marginBottom: 0, marginTop: 0 }}>
                    <li onClick={() => handleClickDetail(data?.canId)}>
                      Xem chi tiết
                    </li>
                    {iconDelete && (
