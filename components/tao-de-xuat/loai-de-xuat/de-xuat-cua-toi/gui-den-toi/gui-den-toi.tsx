...
import styles from './gui-den-toi.module.css'
...
export function GuiDenToi({ dxTo }: { dxTo: any[] }) {
  console.log(dxTo)
  ...
  const items: any = [
    {
      ...
      children: <div>{children(dxTo, columns, router)}</div>,
    },
    {
      key: '2',
      ...
      children: (
        <div>
          {children(
            dxTo?.filter((item) => ![3, 5, 7].includes(item?.type_duyet)),
            columns,
            router
          )}
        </div>
    ...
    },
    {
      ...
      children: (
        <div>
          {children(
            dxTo?.filter((item) => item?.type_duyet === 5),
            columns,
            router
          )}
        </div>
    ...
    },
    {
      ...
      children: (
        <div>
          {children(
            dxTo?.filter((item) => item?.type_duyet === 3),
            columns,
            router
          )}
        </div>
