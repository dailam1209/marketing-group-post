...
import { POST, POST_SS, POST_VT } from '@/pages/api/BaseApi'
...
export const LuanChuyenCongTac: React.FC = () => {
  ...
  const filteredChonChucVu = OPTIONS.filter(
    (x: any) => !selectedItems.includes(x.label)
  )
  const [selectTheoDoi, setSelectTheoDoi] = useState('')
  const theodoi = options2.filter((x: any) => !selectTheoDoi.includes(x?.label))
  const Router = useRouter()
