...
function csvJSON(csv) {
  ...
  headers = headers?.filter(
    (value, index) => value !== '' && value !== ' ' && value !== '\r'
  );
  console.log(headers);
  for (var i = 1; i < lines.length; i++) {
    ...
    if (obj?.[headers[headers?.length - 1]]) {
      result.push(obj);
    }
  }
      ...
      <div className={styles.body}>
              ...
              if (file) {
                Papa.parse(file, {
                  complete: function (results) {
                    setLastFileSelectedJson(results?.data)
        ...
        <div className={styles.khung2}>
          ...
          <div className={styles.link}>
            <CSVLink filename={`fileMau`}
            data={
              [['ID', 'Name', 'BasicSalary', 'ApplyTime'], [135239,'Nguyễn Hiệu Năng', 10000, '2023-08-21']]
