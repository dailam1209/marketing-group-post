...
export const ChuaDuyet = ({ listEmpNoTax, listDepLabel }) => {
  ...
  const [data, setData]: any[] = useState(
    listEmpNoTax?.listUserFinal?.map((emp, index) => {
      const depLabel = listDepLabel?.find(dep => dep?.value === emp?.inForPerson?.employee?.dep_id);
      return {
        key: `${index}`,
        title: emp?.userName,
        id: emp?.idQLC,
  ...
  return (
    <>
      ...
      <div className={styles.khung}>
        ...
        />
        {ModalThiepLapNhanSu(modalAdd, setModalAdd, setModalNext, data, key)}
      </div>
    </>
  );
};
