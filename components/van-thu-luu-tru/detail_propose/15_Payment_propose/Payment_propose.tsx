...
import { post_payment_propose } from '@/utils/api/dexuat/api_post';
import router from 'next/router';

const Payment_propose = ({ inuse }: { inuse?: boolean }) => {
    const validationSchema = Yup.object().shape({
        ...
        so_tien_tt: Yup.number().required('Vui lòng nhập số tiền cần thanh toán.')
      });
    ...
    const handleSubmit = async (e:any) => {
      ...
      try {
        ...
        const res = await post_payment_propose(form_data)
        console.log(res);
        ...
        <form className={styles.form_propose}>
            <div className={styles.wrapper_form}>
                ...
                <div className={styles.row}>
                    <Section
                        style="col_2"
                        label={<Custom_label isRequired={true} title="Số tiền thanh toán" label_class="font_500" />}
                        input={<Custom_input_text inputclass="input_text_shedule" placeholder="Nhập số tiền thanh toán" 
                            input_name='so_tien_tt'
                            handleChange={handleInputChange}
                            type='number' />}
                        validation={errors.so_tien_tt && (
                          <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.so_tien_tt}</p>
                        )}
                    />
                </div>
                <div className={styles.w_100}>
                    <Custom_label isRequired={true} label_class="font_500" title="Lý do đề xuất thanh toán" />
...
export default Payment_propose;