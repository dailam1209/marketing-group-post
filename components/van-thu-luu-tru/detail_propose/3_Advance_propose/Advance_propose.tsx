...
import { post_abvance_propose } from '@/utils/api/dexuat/api_post';
import router from 'next/router';
import Input_calender from '../../components/Input/Input_calender/Input_calender';
import { Input_file_3 } from '../../components/Input/Input_file/Input_file';
import Input_select from '../../components/Input/Input_select/Input_select';
import { Custom_input_text, Custom_input_textarea } from '../../components/Input/Input_text/Input_text';
import { Custom_label } from '../../components/Input/Label/Label';
import Section from '../../components/Input/Section/Section';

const Advance_propose = ({inuse}:{inuse?:boolean}) => {
  const validationSchema = Yup.object().shape({
    ...
    ngay_tam_ung: Yup.string().required('Vui lòng nhập ngày tạm ứng.'),
    tien_tam_ung: Yup.string().required('Vui lòng nhập số tiền tạm ứng'),
  });
    ...
    const handleSubmit = async (e:any) => {
      ...
      try {
        ...
        const res = await post_abvance_propose(form_data)
                    ...
                    <div className={styles.col_2_d_flex}>
                        ...
                        <Section 
                            ...
                            input={<Custom_input_text 
                                    ...
                                    value="Đơn xin tạm ứng" 
                            />} 
                        />
                    </div>
                    <Section 
                        style="col_2" 
                        label={<Custom_label isRequired={true} title="Ngày tạm ứng" label_class="font_500" />} 
                        input={<Input_calender 
                                    placeholder="Chọn ngày" 
                                    datetype='date'
                                    calender_class="calender_class_schedule"
                                    calender_label_class="calender_label_class_schedule"
                                    input_name='ngay_tam_ung'
                                    handle_input={handleInputChange}
                                />} 
                        validation={errors.ngay_tam_ung && (
                          <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.ngay_tam_ung}</p>
                        )}
                    />
                    <div className={styles.col_2}>
                        <Custom_label isRequired={true} title="Số tiền tạm ứng" label_class="font_500" />
                        ...
                        {errors.tien_tam_ung && (
                          <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.tien_tam_ung}</p>
                        )}
                    </div>
                </div>
                <div className={styles.w_100}>
                    <Custom_label isRequired={true} label_class="font_500" title="Lý do xin tạm ứng" />
...
export default Advance_propose;