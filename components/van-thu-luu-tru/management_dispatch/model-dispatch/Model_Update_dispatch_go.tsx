...
const Modal_dis_update_go: React.FC<ModalProps> = ({
  isOpen,
  onClose,
  title,
  item,
  ...
  const [typeLoaiVb, setTypeLoaiVb] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (item) {
      ...
      let typevbDi = item?.cv_kieu.split(" ").map(Number);
      setTypeLoaiVb(typevbDi);
  ...
  const handleCheckboxChange = (value: any) => {
    if (typeLoaiVb?.includes(value)) {
      setTypeLoaiVb((prev) => prev.filter((val) => val !== value));
    } else {
      setTypeLoaiVb((prev) => [...prev, value]);
    }
  };
                  ...
                  {options_type_loai_vb.map((option) => (
                    <label key={option.value} className={styles.label_type}>
                      <input
                        ...
                        name="type_loai_vb"
                        value={option.value}
                        checked={typeLoaiVb?.includes(option.value)}
...
export default Modal_dis_update_go;
