...
import { post_pregnant_propose } from '@/utils/api/dexuat/api_post';
import router from 'next/router';

const Pregnant_propose = ({ inuse }: { inuse?: boolean }) => {
    const validationSchema = Yup.object().shape({
        ...
        ngaybatdau_nghi_ts: Yup.string().required('Vui lòng nhập ngày bắt đầu nghỉ.'),
        ngayketthuc_nghi_ts: Yup.string().required('Vui lòng nhập ngày kết thúc nghỉ.')
      });
        ...
        <div className={styles.wrapper_form}>
            <div className={styles.row}>
                ...
                <div className={styles.col_2_d_flex}>
                    ...
                    <Section
                        ...
                        input={<Custom_input_text
                            ...
                            value="Đề xuất xin nghỉ chế độ thai sản"
                        />}
                    />
                </div>
               ...
               <Section 
                    ...
                    input={<Input_select 
                        style={select_style}
                        placeholder={'Kiểm duyệt lần lượt'}
                        options={confirm_type_options}
                    />}
                />
                 <Section style='col_2'
                    label={<Custom_label isRequired={false} label_class="font_500" title="Thời gian nghỉ từ ngày" />}
                    input={<Input_calender
                        placeholder='Chọn ngày '
                        datetype='date'
                        calender_class="calender_class_schedule"
                        calender_label_class="calender_label_class_schedule"
                        input_name='ngaybatdau_nghi_ts'
                        handle_input={handleInputChange}
                    />}
                    validation={errors.ngaybatdau_nghi_ts && (
                      <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.ngaybatdau_nghi_ts}</p>
                    )}
                />
                  <Section style='col_2'
                    label={<Custom_label isRequired={false} label_class="font_500" title="Đến ngày" />}
                    input={<Input_calender
                        placeholder='Chọn ngày '
                        datetype='date'
                        calender_class="calender_class_schedule"
                        calender_label_class="calender_label_class_schedule"
                        input_name='ngayketthuc_nghi_ts'
                        handle_input={handleInputChange}
                    />}
                    validation={errors.ngayketthuc_nghi_ts && (
                      <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.ngayketthuc_nghi_ts}</p>
                    )}
                />
            </div>
            <div className={styles.w_100}>
                <Custom_label isRequired={true} label_class="font_500" title="Lý do đề xuất xin nghỉ chế độ thai sản" />
            ...
            <div className={`${styles.mt_15} ${styles.row}`}>
                    ...
                    <Section 
                        ...
                        input={<Input_file_3 placeholder="Thêm tài liệu đính kèm"
                            name='fileKem' handleChange={handleFileChange} />} 
                    />
                </div>
        </div>
...
export default Pregnant_propose;