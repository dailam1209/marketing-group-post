...
import { post_transportation_propose } from '@/utils/api/dexuat/api_post';
import router from 'next/router';

const Transportation_propose = ({ inuse }: { inuse?: boolean }) => {
    const validationSchema = Yup.object().shape({
        ...
        soluong_xe: Yup.string().required('Vui lòng nhập số lượng xe.'),
        local_di: Yup.string().required('Vui lòng nhập địa điểm đi.'),
        local_den: Yup.string().required('Vui lòng nhập địa điểm đến.'),
        bd_xe: Yup.string().required('Vui lòng nhập thời gian đi.'),
        end_xe: Yup.string().required('Vui lòng nhập thời gian đến.'),
      });
    ...
    const handleSubmit = async (e:any) => {
      ...
      try {
        ...
        const res = await post_transportation_propose(form_data)
        console.log(res);
        ...
        <form className={styles.form_propose}>
            <div className={styles.wrapper_form}>
                <div className={styles.row}>
                    <Section
                        ...
                        input={<Custom_input_text inputclass="input_text_shedule" placeholder="Nhập tên đề xuất"
                            input_name='name_dx' handleChange={handleInputChange} />}
                    ...
                    <div className={styles.col_2_d_flex}>
                        ...
                        <Section
                            ...
                            input={<Custom_input_text
                                ...
                                inputclass="input_text_shedule"
                                placeholder=""
                                value="Đề xuất đăng ký sử dụng xe công"
                            />}
                        />
                    </div>
                </div>
                <div className={styles.row}>
                    <Section
                        style="col_2"
                        label={<Custom_label isRequired={false} title="Số lượng xe sử dụng" label_class="font_500" />}
                        input={<Custom_input_text inputclass="input_text_shedule" placeholder="Nhập số lượng xe"
                        input_name="soluong_xe"
                        handleChange={handleInputChange}
                        type='number' />}
                        validation={errors.soluong_xe && (
                            <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.soluong_xe}</p>
                        )}
                    />
                </div>
                <div className={styles.row}>
                    <div className={styles.col_2_d_flex}>
                        <Section
                            style="w_50_g20"
                            label={<Custom_label isRequired={true} label_class="font_500" title="Thời gian đi" />}
                            ...
                            />}
                            validation={errors.local_di && (
                                <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.local_di}</p>
                            )}
                        />
                        <Section
                            style="w_50_g20"
                            ...
                            input={<Input_calender
                                ...
                                handle_input={handleInputChange}
                            />}
                            validation={errors.bd_xe && (
                                <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.bd_xe}</p>
                            )}
                        />
                    </div>
                    <div className={styles.col_2_d_flex}>
                        <Section
                            style="w_50_g20"
                            label={<Custom_label isRequired={true} label_class="font_500" title="Thời gian đến" />}
                            input={<Input_calender
                                placeholder="Chọn thời gian đến"
                                datetype="time"
                                calender_class="calender_class_schedule"
                                calender_label_class="calender_label_class_schedule"
                                input_name='local_den'
                                handle_input={handleInputChange}
                            />}
                            validation={errors.local_den && (
                                <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.local_den}</p>
                            )}
                        />
                        <Section
                            style="w_50_g20"
                            ...
                            input={<Input_calender
                                ...
                                handle_input={handleInputChange}
                            />}
                            validation={errors.end_xe && (
                                <p style={{color: 'red', margin: '10px 10px 10px 0'}} >{errors.end_xe}</p>
                            )}
                        />
                    </div>
                </div>
                <div className={styles.w_100}>
                    <Custom_label isRequired={true} label_class="font_500" title="Lý do đề xuất xin sử dụng xe công" />
...
export default Transportation_propose;