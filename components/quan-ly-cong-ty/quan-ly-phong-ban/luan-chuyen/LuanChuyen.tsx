...
export function LuanChuyen({ listTranferJob, listDepartments, listTeams, listGroups, infoCom }) {
  ...
  const [listTeamLabel, setTeamLabel]: any = useState(listTeams?.data?.map(team => ({ label: team?.team_name, value: team?.team_id })))
  const [listGrLabel, setListGrLabel]: any = useState(listGroups?.data?.map(group => ({ label: group?.gr_name, value: group?.gr_id })))
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: any, index: any) => <p>{record?.note}</p>,
    },
    {
    ...
    },
    {
      ...
      render: (record: any, index: any) => <p>{record?.old_com_name || "Chưa cập nhật"}</p>,
    },
    {
  ...
  return (
    <div>
      ...
      />
      {UpdatePhongBanModal(openEdit, setOpenEdit, data, setData, selectedRow, listDepLabel, infoCom, listTeamLabel, listGrLabel)}
      {ConfirmDeleteModal(
        openConfirmDel,
        setOpenConfirmDel,
      ...
      {AddNewModal(openAddNew, setOpenAddNew, setData, listEmp, listDepLabel, listTeams?.data, listGroups?.data)}
    </div>
  );
}
