...
export function GiamBienChe({ listQuitJob, listDepartments, infoCom }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: any, index: any) => <p>{record?.time}</p>
    },
    {
    ...
    <div>
      ...
      <Row gutter={{lg:20,md:25,sm:20,xs:20}}>
        ...
        <Col lg={{span:11,order:1}} md={{span:10,order:2}} sm={12} xs={24}>
          <div>{mySelect(true,"", "Nhập tên nhân viên", true, false,"ep_id", data?.map(emp => ({ label: emp?.ep_name, value: emp?.ep_id })))}</div>
        </Col>
        <Col lg={{span:3,order:1}} md={{span:4,order:3}} sm={{span:14,order:2}} xs={{span:12,order:1}} className={styles.buttonLeft}>
        <Row gutter={20} justify={'end'}>
          <Col className={styles.button}><div>{SearchButton("Tìm kiếm", () => null, false)}</div></Col>
          <Col className={styles.buttonCenter1}>{ExportExcelButton(() => null)}</Col>
        </Row>
        </Col>
        ...
        <Col lg={{span:14,order:1}} md={{span:14,order:5}} sm={{span:24,order:3}} xs={{span:12,order:2}}>
        <Row gutter={20} justify={'end'}>
          <Col className={styles.buttonCenter2}>
            {ExportExcelButton(() => null)}
          </Col>
          <Col className={styles.buttonRight}>
            <div>{AddButton("Thêm mới giảm biên chế", () => setOpenAddNew(true))}</div>
          </Col>
        </Row>
        </Col>
      ...
      />
      {UpdatePhongBanModal(openEdit, setOpenEdit,data, setData, selectedRow, companyLabel, listDepLabel)}
      {ConfirmDeleteModal(
        openConfirmDel,
        setOpenConfirmDel,
      ...
      {AddNewModal(openAddNew, setOpenAddNew, setData, listNV, setListNV, companyLabel, listDepLabel)}
    </div>
  )
}
