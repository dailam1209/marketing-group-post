...
class CallApi {
    ...
    static async functionAPI(token, data, url) {
        let configHeader = {
            headers: {}
        };
        if (token) {
    ...
    // api check vip
    static async checkVipById(idCom) {
        const result = 
        let response = '';
        const call = await axios.post('http://210.245.108.202:3000/api/checkVip/before', { idCom: idCom });
        response = call;
        return response;
    }
